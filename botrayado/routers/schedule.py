...
async def get_schedule(event: SimpleBotEvent, button: str) -> str:
...
async def get_group(event: SimpleBotEvent, fetch: dict, flag: bool, btn: str, button: str) -> str:
    try:
        ...
        # Пред предпоследняя команда
        pre_penultimate_command = fetch[3][0].lower()
        # Пред пред предпоследняя команда
        pre_pre_penultimate_command = fetch[4][0].lower()
    except IndexError as e:
        logger.error(
            f"Error in schedule.py, fetching commands ({e}): {traceback.format_exc()}")
    ...
    if flag == [(0,)]:
        if any(cmd.lower() in [pre_penultimate_command] for cmd in schedule_kb.CURRENT_OR_NEXT_WEEK_BUTTONS) and pre_pre_penultimate_command == schedule_kb.DAYS_OF_WEEK_BUTTONS[2]:
            ...
            await event.answer(message=schedule, keyboard=menu_kb.create_menu_keyboard(button).get_keyboard())
            ...
            await event.answer(message="Не нажимайте на 2 разные кнопки в одной категории. Повторите свой запрос.", keyboard=menu_kb.create_menu_keyboard(button).get_keyboard())
    else:
        if any(cmd.lower() in [pre_penultimate_command] for cmd in schedule_kb.CURRENT_OR_NEXT_WEEK_BUTTONS) and pre_pre_penultimate_command == schedule_kb.DAYS_OF_WEEK_BUTTONS[2]:
            # ТН БВТ2103 либо СН БВТ2103 (текущая неделя | следующая неделя, группа)
            result = pre_penultimate_command.split(
            )[0][0].upper() + 'Н' + ' ' + last_command.upper()
        elif any(cmd.lower() in [pre_penultimate_command] for cmd in schedule_kb.DAYS_OF_WEEK_BUTTONS):
            # Сегодня БВТ2103 (сегодня | завтра, группа)
            result = pre_penultimate_command.capitalize() + ' ' + last_command.upper()
        else:
            await event.answer(message="Непредвиденная ошибка.", keyboard=menu_kb.create_menu_keyboard(button).get_keyboard())
        already_exists = set_button_blueprint(btn[0][0], result, event)
        await event.answer(message=f"Задан шаблон: {result}", keyboard=menu_kb.create_menu_keyboard(button).get_keyboard()) if not already_exists else await event.answer(message="Данный шаблон уже существует в {} ячейке.".format({'first_btn': 1, 'second_btn': 2, 'third_btn': 3}[btn[0][0]]), keyboard=menu_kb.create_menu_keyboard(button).get_keyboard())
